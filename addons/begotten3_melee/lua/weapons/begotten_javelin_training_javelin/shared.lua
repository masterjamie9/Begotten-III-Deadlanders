...
SWEP.AttackSoundTable = "BluntMetalJavelinMeleeAttackSoundTable" 
	...
	if SERVER and owner:IsPlayer() then
		...
		if owner.opponent then
			...
			if owner.StripWeapon then
				owner:StripWeapon("begotten_javelin_training_javelin");
			end
		else
...
SWEP.WElementsAlternate = {
	["w_training_javelin"] = { type = "Model", model = "models/begotten/weapons/training_spear.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(2.596, 1.5, 2.596), angle = Angle(8.182, 17.531, -174.157), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}