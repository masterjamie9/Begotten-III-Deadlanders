...
SWEP.PrintName = "Throwing Dagger"
...
SWEP.AttackSoundTable = "MetalThrowingDaggerMeleeAttackSoundTable" 

/*---------------------------------------------------------
	PrimaryAttack
---------------------------------------------------------*/

SWEP.AttackTable = "ThrowingDaggerAttackTable"
...
function SWEP:HandlePrimaryAttack()
	...
	timer.Create("javelin_timer_"..self.Owner:EntIndex(), 0.5, 1, function()
		if IsValid(self) and IsValid(self.Owner) and self.Owner:GetActiveWeapon() and self.Owner:GetActiveWeapon().PrintName == "Throwing Dagger" then
			self:FireJavelin();
...
function SWEP:FireJavelin()
	...
	if SERVER then
		...
		javelin:SetModel("models/demonssouls/weapons/cut throwing dagger offset.mdl");
	...
	if SERVER and owner:IsPlayer() then
		...
		if owner.opponent then
			...
			if owner.StripWeapon then
				owner:StripWeapon("begotten_javelin_throwing_dagger");
			end
		else
...
SWEP.VElements = {
	["v_throwing_dagger"] = { type = "Model", model = "models/demonssouls/weapons/cut throwing dagger.mdl", bone = "v_weapon.Knife_Handle", rel = "", pos = Vector(0,0,5), angle = Angle(110,40,180), size = Vector(1,1,1), material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_throwing_dagger"] = { type = "Model", model = "models/demonssouls/weapons/cut throwing dagger.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3, 1, -0.9), angle = Angle(-100, -110, 0), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}