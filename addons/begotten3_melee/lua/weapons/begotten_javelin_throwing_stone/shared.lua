...
SWEP.AttackTable = "ThrowingStoneAttackTable"
...
function SWEP:HandlePrimaryAttack()
	...
	timer.Create("javelin_timer_"..self.Owner:EntIndex(), 0.5, 1, function()
		if IsValid(self) and IsValid(self.Owner) and self.Owner:GetActiveWeapon() and self.Owner:GetActiveWeapon().PrintName == "Throwing Stone" then
			self:FireJavelin();
...
function SWEP:FireJavelin()
	...
	if SERVER then
		...
		javelin:SetModel("models/props_mining/rock_caves01b.mdl");
		javelin:SetAngles(owner:GetAimVector():Angle() + Angle(90, 0, 0))
		javelin:SetPos(pos)
	...
	if SERVER and owner:IsPlayer() then
		...
		if owner.opponent then
			...
			if owner.StripWeapon then
				owner:StripWeapon("begotten_javelin_throwing_stone");
			end
		else
...
SWEP.VElements = {
	["v_throwing_stone"] = { type = "Model", model = "models/props_mining/rock_caves01b.mdl", bone = "v_weapon.Knife_Handle", rel = "", pos = Vector(-0.5, 1.5, 5.5), angle = Angle(125, 0, -30), size = Vector(0.75, 0.75, 0.75), material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_throwing_stone"] = { type = "Model", model = "models/props_mining/rock_caves01b.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(6, 0, 0), angle = Angle(-8, 0, 100), size = Vector(1, 1, 1), material = "", skin = 0, bodygroup = {} }
}