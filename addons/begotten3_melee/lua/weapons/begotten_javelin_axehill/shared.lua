...
SWEP.AttackTable = "HillThrowingAxeAttackTable"

SWEP.Primary.Round = ("begotten_javelin_axehill_thrown");
	...
	if SERVER and owner:IsPlayer() then
		...
		if owner.opponent then
			...
			if owner.StripWeapon then
				owner:StripWeapon("begotten_javelin_axehill");
			end
		else
...
SWEP.VElements = {
	["v_pilum"] = { type = "Model", model = "models/begotten_apocalypse/items/WinterholdAxe.mdl", bone = "v_weapon.Knife_Handle", rel = "", pos = Vector(0,0,5), angle = Angle(110,40,180), size = Vector(1,1,1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}

SWEP.WElements = {
	["w_pilum"] = { type = "Model", model = "models/begotten_apocalypse/items/WinterholdAxe.mdl", bone = "ValveBiped.Bip01_R_Hand", rel = "", pos = Vector(3, 0.2, -0.91), angle = Angle(-90,0,90), size = Vector(1, 1, 1), color = Color(255, 255, 255, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}